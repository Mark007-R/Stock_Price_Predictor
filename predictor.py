...
# Job store: { job_id: { "status": "running"|"done"|"error", "result": {...}, "error": str } }
_jobs = {}
...
def run_prediction(job_id, ticker, num_days):
    """Runs in a background thread; stores result in _jobs."""
    try:
        data = fetch_stock_data(ticker, period="2y")
        if len(data) < 100:
            raise Exception(f"Insufficient data for {ticker}: {len(data)} days found, need 100+.")
        data = data.dropna()
        ...
        logger.error(f"[{job_id}] Prediction failed: {str(e)}", exc_info=True)
...
def predict():
    ...
    # Show loading page that polls for result
